...
def build_datafile_name(dic):
    ...
    #output: 
    event_number = str(dic['event_number'])  # 假设 'event_number' 是从字典 'dic' 中取出的
    start_time = str(dic['start_time'])      # 同上，确保这些字段存在于字典中
    duration = str(dic['duration'])
    channel = str(dic['channel'])     

    wav = '../events/waveforms/0000'+event_number+'_'+start_time+'_'+duration+'_'+channel+'s.sac'
    spec = '../events/spectrograms/data/0000'+event_number+'_'+start_time+'_'+duration+'_'+channel+'s.txt'
    return wav, spec
...
if __name__ == '__main__':
    ...
    f = open('E:/data/out/features_loss.dat', 'rb')
    ...
    r.to_excel('E:/data/out//Cluster centers.xlsx')
    ...
    for cluster_id in A['cluster'].unique():
        cluster_data = A[A['cluster'] == cluster_id]
        filename = f'E:/data/out/Cluster_{cluster_id}_Data.xlsx'
        cluster_data.to_excel(filename, index=False)
        print(f'Saved {filename} with {len(cluster_data)} records.')
    
    print('Clustering and saving completed!')
    
    # A.to_excel('E:/data/out//Total_Clusters.xlsx')
    # print('Clustering over!')
    
    # A = pd.read_excel('E:/data/out//Total_Clusters.xlsx')
